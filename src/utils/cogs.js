...
// Profit for a single invoice line: (SalePrice - UnitCost) * Quantity.
// Prefers a unitCost already snapshotted on the line (captured at sale time)
// so historic invoices don't drift if batch costs change later; falls back
// to a live FIFO lookup for older invoices saved before this snapshot existed.
export function calcLineProfit(line, product, batches) {
  const qty = Number(line.qty) || 0;
  const salePrice = Number(line.unitPrice) || 0;
  const unitCost = line.unitCost ?? getFIFOUnitCost(line.productId, batches, product);
  return (salePrice - unitCost) * qty;
}
